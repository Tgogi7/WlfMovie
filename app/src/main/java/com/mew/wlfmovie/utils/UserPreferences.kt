...
import com.mew.wlfmovie.providers.Provider.Companion.providers
...
object UserPreferences {
    ...
    // =====================================================================
    // WLFMOVIE: Idioma por defecto de la app = español (es).
    // TMDB siempre se usa como provider de catálogo en español.
    // =====================================================================
    private const val WLFMOVIE_DEFAULT_LANGUAGE = "es"
    ...
    fun setup(context: Context) {
        ...
        if (::prefs.isInitialized) {
            ...
            // WLFMOVIE: Forzar TMDb (es) como provider en el primer arranque,
            // o si por algun motivo el provider guardado ya no existe en la lista.
            val currentName = Key.CURRENT_PROVIDER.getString()
            if (currentName.isNullOrBlank() || currentName == "null") {
                Key.CURRENT_PROVIDER.setString("TMDb (Español)")
                Log.i(TAG, "WLFMOVIE: provider por defecto establecido = TMDb (Español)")
            }
        }
    }
    ...
    var currentProvider: Provider?
        get() {
            // WLFMOVIE: Siempre devolver TMDb en español a menos que el usuario
            // haya seleccionado explicitamente un provider distinto y valido.
            val providerName = Key.CURRENT_PROVIDER.getString()

            // Si es TMDb (cualquier idioma), respetar el idioma guardado
            if (providerName?.startsWith("TMDb (") == true && providerName.endsWith(")")) {
                val lang = providerName.substringAfter("TMDb (").substringBefore(")")
                // Si el idioma viene como "Español" (display name), mapearlo a "es"
                val resolvedLang = resolveLanguageCode(lang)
                return TmdbProvider(resolvedLang)
            }

            // Si es un provider normal valido, devolverlo
            val resolved = Provider.providers.keys.find { it.name == providerName }
            if (resolved != null) return resolved

            // Default: TMDb (es)
            return TmdbProvider(WLFMOVIE_DEFAULT_LANGUAGE)
        }
        set(value) {
    ...
    /**
     * Mapea display names de idioma ("Español", "Inglés", etc.) a codigos ISO ("es", "en").
     * Esto porque ProvidersViewModel guarda "TMDb (Español)" en vez de "TMDb (es)".
     */
    private fun resolveLanguageCode(displayNameOrCode: String): String {
        return when (displayNameOrCode.lowercase()) {
            "es", "español", "espanol", "spanish" -> "es"
            "en", "inglés", "ingles", "english" -> "en"
            "it", "italiano", "italian" -> "it"
            "de", "alemán", "aleman", "german" -> "de"
            "fr", "francés", "frances", "french" -> "fr"
            else -> displayNameOrCode // Asumir que ya es un codigo ISO
        }
    }
    ...
    var providerLanguage: String?
        get() = Key.PROVIDER_LANGUAGE.getString() ?: WLFMOVIE_DEFAULT_LANGUAGE
    ...
    enum class PlayerResize(
        ...
        companion object {
            fun fromName(name: String?): PlayerResize =
                entries.find { it.name == name } ?: Fit
        }
    }
